...
# --- Firebase Test Write ---
# st.subheader("🔧 Firebase Connectivity Test")
# if st.button("Test Firestore Write"):
#     try:
#         result = db.collection("connectivity_test").add({
#             "message": "Test write from Streamlit!",
#             "timestamp": firestore.SERVER_TIMESTAMP,
#             "user": st.session_state.get("user_email", "unknown")
#         })
#         st.success(f"✅ Firestore write successful! Document ID: {result[1].id}")
...
if INACTIVITY_LIMIT < inactive_time <= INACTIVITY_LIMIT + WARNING_DURATION:
    ...
    for entry in st.session_state.log_data:
        try:
            ...
            st.error("Error writing tracking log to Firebase")
            st.exception(e)
    ...
    try:
        ...
        st.error("Error writing session summary to Firebase")
        st.exception(e)
