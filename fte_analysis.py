...
def calculate_average_q2_q4_time(df, fte_change, occupancy_assumption):
    # Adjust FTE by the given amount using the mean of Staffing Requirement (ACTUAL)
 
    df['Adjusted_FTE'] = df['FTE Requirement'].mean() + fte_change
    
    # Change occupancy assumption to the given value
    df['Adjusted_Occupancy_Assumption'] = occupancy_assumption
    
    # Filter entries with FTE within ±10% and occupancy assumption within ±10%
    fte_lower_bound = df['Adjusted_FTE'] * 0.9
    fte_upper_bound = df['Adjusted_FTE'] * 1.1
    occupancy_lower_bound = df['Adjusted_Occupancy_Assumption'] * 0.9
    occupancy_upper_bound = df['Adjusted_Occupancy_Assumption'] * 1.1
    

    filtered_df = df[(df['FTE Requirement'] >= fte_lower_bound) & (df['FTE Requirement'] <= fte_upper_bound) &
...
def calculate_average_occupancy_rate(df, fte_change, occupancy_assumption):
    # Adjust FTE by the given amount using the mean of FTE Requirement
    df['Adjusted_FTE'] = df['FTE Requirement'].mean() + fte_change
...
def group_data(df, min_value, max_value):
    ...
    df['Requirement_Range'] = pd.cut(df['FTE Requirement'], bins=bins)
...
def run_fte_analysis():
    ...
    if uploaded_file is not None:
        ...
        df = df[df['FTE Requirement'] != 0]
        df = df[df['FTE Requirement'] != '']
        ...
        # User inputs for FTE change and occupancy assumption
        fte_change_1 = st.number_input("Enter the change in FTE for scenario 1:", value=0)
        occupancy_assumption_1 = st.number_input("Enter the occupancy assumption for scenario 1 (as a percentage):", value=100) / 100
        
        fte_change_2 = st.number_input("Enter the change in FTE for scenario 2:", value=0)
        occupancy_assumption_2 = st.number_input("Enter the occupancy assumption for scenario 2 (as a percentage):", value=100) / 100       
        
        # Calculate average Q4 time and occupancy rate with FTE reduced by 50 and occupancy assumption of 60%
        average_q2_q4_time_1 = calculate_average_q2_q4_time(df, fte_change_1, occupancy_assumption_1)
        average_occupancy_rate_1 = calculate_average_occupancy_rate(df, fte_change_1, occupancy_assumption_1)

        # Calculate average Q4 time and occupancy rate with FTE reduced by 80 and occupancy assumption of 70%
        average_q2_q4_time_2 = calculate_average_q2_q4_time(df, fte_change_2, occupancy_assumption_2)
        average_occupancy_rate_2 = calculate_average_occupancy_rate(df, fte_change_2, occupancy_assumption_2)

        # Display results in specified format
        st.write("Whatif Results------------>")
        results = pd.DataFrame({
            'FTE Requirement': [df['FTE Requirement'].mean() + fte_change_1,
                                              df['FTE Requirement'].mean() + fte_change_2],
        ...
        }, index=[f'Change by {fte_change_1} FTE', f'Change by {fte_change_2} FTE'])

        st.write(results)
        
        st.write(df)
        min_value = int(df['FTE Requirement'].min())
        max_value = int(df['FTE Requirement'].max())
        grouped_results = group_data(df, min_value, max_value)
        st.write("Data distribution based on FTE buckets-------->")
